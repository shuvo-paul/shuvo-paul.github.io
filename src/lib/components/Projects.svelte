...
<div class="pt-8 mt-32" id="projects">
    <h2 class="unstyled text-2xl mb-9">{title}</h2>
    ...
    {#each list as item, i }
        <div class="grid grid-cols-12 gap-x-8 gap-y-4 items-center mb-20">
            ...
            <div class={`col-span-full md:row-span-full ${(i+1)%2? 'md:col-span-7 md:col-end-13 md:text-right' : 'md:col-start-1 md:col-span-7' } z-10`}>
                {#if item.title}
                    <h3 class="text-primary-500 text-xl mb-4 mx-3">{item.title}</h3>
                {/if}

                {#if item.content}
                    <div class="bg-surface-600 bg-opacity-95 shadow p-6 rounded">
                        {item.content}
                    </div>
                {/if}
                ...
                <ul class={`mt-4 flex list-none ${(i+1)%2? 'md:justify-end' : '' }`}>
                    {#if item.gitURL}
                        <li class="mx-3">
                            <a class="unstyled hover:text-primary-500" target="_blank" href={"//"+item.gitURL} rel="noreferrer">
                                <Github class="icon" />
                            </a>
                        </li>
                    {/if}
                    ...
                    {#if item.externalURL }
                        <li class="mx-3">
                            <a class="unstyled hover:text-primary-500" target="_blank" href={"//"+item.externalURL} rel="noreferrer">
